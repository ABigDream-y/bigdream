...
export interface IViewHooksOptions {
	...
	* */
	routeName ?: string;
	routeTitle ?: string;
	routeTitleCustom ?: string;
	routePath ?: string;

	/**
	 * 分类列表
	 * */
	getTypeListURL ?: string;
	typeList ?: IObject[];
...
export interface IViewHooks extends IViewHooksOptions, IObject {
	 ...
	 * 
	 * **/
	isFormCreator : (creator : string) => boolean;
	/**
	 * 消息推送辅导员操作是否显示
	 * ***/
	showPutMsgAction : () => boolean;
	/**
	 * 浏览数量转换
	 */
	getVisitNumber : (number : number) => string | number;
	/**
	 * 检查权限
	 */
	...
	editWidthParamsHandle : (query ?: IObject) => void;
	/**
	 * 列表删除事件
	 */
