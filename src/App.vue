	...
	export default defineComponent({
		...
		setup() {
			...
			const state = reactive({
				layout: location.href.includes("pop=true") ? EPageLayoutEnum.fullscreen : EPageLayoutEnum
					.page
			});
			...
			watch(
				...
				([path, query, fullPath]) => {
					store.dispatch({
						type: "updateState",
						payload: {
							activeTabName: fullPath
						}
					});
					state.layout = app.fullscreenPages.includes(path as string) || (query as IObject)["pop"] ?
						EPageLayoutEnum.fullscreen : EPageLayoutEnum.page;
				}
			);
			return {
				store,
...
<template>
	<el-config-provider :locale="locale">
		<div v-if="!store.state.appIsRender" v-loading="true" :element-loading-fullscreen="true"
