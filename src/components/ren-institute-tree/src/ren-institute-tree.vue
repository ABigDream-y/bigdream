<template>
  <div>
    <el-select v-if="showType == 'select'" :size="size" v-model="value" :filterable="filterable" @change="selectChange" :placeholder="placeholder" :disabled="disabled" clearable>
    ...
    <div v-else>
      ...
      <el-dialog v-model="visibleDept" width="30%" :modal="false" :title="placeholder" :close-on-click-modal="false" :close-on-press-escape="false">
        ...
        <el-tree class="filter-tree" :data="deptList" :default-expanded-keys="expandedKeys" :props="{ label: 'name', children: 'children' }" :expand-on-click-node="false" :filter-node-method="filterNode" :highlight-current="true" :node-key="nodekey" ref="tree"> </el-tree>
  ...
  methods: {
    ...
    selectChange(val: string) {
      ...
      if (index != -1) {
        ...
        this.$emit("update:deptName", this.deptList[index]["name"]);
      } else {
