	...
	export default defineComponent({
		...
		setup(props) {
			const getStyle = (index: number): string => {
				...
				const isHidden = props.hiddenIndex ? props.hiddenIndex > -1 && index > props.hiddenIndex :
					false;
...
<template>
	<template v-for="(x, index) in props.menus || []" :key="x.path">
		<el-sub-menu v-if="x.children && x.children.length > 0" :index="x.path" :popper-class="props.className"
			:class="classNames({ isMore: x.meta?.isMore })" :style="getStyle(index)">
			<template #title>
		...
		<el-menu-item v-else :index="x.meta?.isNewPage ? x.path : x.path"
			:class="classNames({ isLink: !!x.meta?.isNewPage, isMore: x.meta?.isMore })" :style="getStyle(index)">
			<template #title>
