<script lang="ts">
	...
	export default defineComponent({
		...
		setup(props : IExpand) {
			...
			watch(
				() => store.state.appIsLogin && store.state.user.staffRole == '辅导员',
				(vl) => {
					if (vl) {
			...
			// 	const {
			// 		data
			// 	} = useWebSocket(app.websocket + "?token=" + getToken(), {
			// 		autoReconnect: true,
			// 		heartbeat: {
			// 			interval: 15000
			// 		},
			...
			const messagePermission = computed(() => checkPermission(store.state.permissions, "sys:notice:all")) && store.state.user.staffRole == '辅导员';
			return {
				props,
				store,
				isFullscreen,
...
<template>
	<div class="rr-header-right-items">
		...
		<div style="display: flex; justify-content: center; align-items: center">
			<img :src="userLogo" :alt="props.userName"
