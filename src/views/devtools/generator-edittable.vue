<template>
	<el-dialog v-model="visible" title="编辑" :close-on-click-modal="false" :close-on-press-escape="false"
		:fullscreen="true">
		<vxe-table border row-key ref="xTable" class="sortable-row-gen" size="small" :data="tableData"
			...
			<vxe-colgroup title="字段" align="center">
				...
				<vxe-column field="columnComment" width="140" title="字段说明"
				...
				<vxe-column field="dictName" width="140" title="字典名称">
					<template v-slot="{ row }">
						<vxe-select v-model="row.dictName">
							<vxe-option v-for="item in dictList" :key="item.value" :value="item.value"
								:label="item.label"></vxe-option>
						</vxe-select>
					</template>
				</vxe-column>
		...
		methods: {
			...
			getTable(id : string) {
				baseService.get("/devtools/table/" + id).then((res) => {
					...
					// let dataList = this.tableData.map((item:IObject)=>{
					// 	return {
					// 		field: item.columnName,
					// 		filedName: item.columnComment,
