...
activated() {
  ...
  methods: {
    ...
    // 获取信息
    getInfo() {
      baseService.get("/task/tasksettinginstitute/" + this.dataForm.id).then((res) => {
        if (res.code !== 0) {
    ...
    dataFormSubmitHandle() {
      this.$refs["dataFormRef"].validate((valid: boolean) => {
		...
		this.loading = true;
        (!this.dataForm.id ? baseService.post : baseService.put)("/task/tasksettinginstitute", this.dataForm).then((res) => {
          if (res.code !== 0) {
			this.loading = false;
            return this.$message.error(res.msg);
          }
