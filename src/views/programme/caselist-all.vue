<template>
  <div class="mod-programme__caselist-all">
    <el-form :inline="true" ref="searchForm" :model="dataForm" @keyup.enter="getDataList()">
      ...
      <el-form-item prop="major" v-if="dataForm.instituteNo">
        <el-select v-model="dataForm.major" placeholder="专业">
          <el-option @click="majorChange(item)" v-for="(item, index) in majorList" :key="index" :value="item.name" :label="item.name"></el-option>
        </el-select>
      </el-form-item>
    ...
    <el-table v-loading="dataListLoading" :data="dataList" :default-sort="defaultSort" border @sort-change="dataListSortChangeHandle" @selection-change="dataListSelectionChangeHandle" style="width: 100%">
      ...
      <el-table-column prop="createDate" label="提交时间" sortable header-align="center" align="center" width="200"></el-table-column>
      <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
        <template v-slot="scope">
          <el-button v-if="hasPermission('programme:caselist:update') && (scope.row.status == 0 || scope.row.status == 3)" type="text" size="small" @click="editHandle(scope.row.id)">{{ $t("update") }}</el-button>
          <el-button v-else type="text" size="small"
          	@click="detailHandle(scope.row)">详情</el-button>
          <el-button v-if="hasPermission('programme:caselist:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t("delete") }}</el-button>
          <el-button v-if="scope.row.status == 2" type="text" size="small" @click="multiAuditHandle(scope.row.id, 1)">通过</el-button>
          <el-button v-if="scope.row.status == 2" type="text" size="small" @click="multiAuditHandle(scope.row.id, 2)">不通过</el-button>
        </template>
      </el-table-column>
    </el-table>
