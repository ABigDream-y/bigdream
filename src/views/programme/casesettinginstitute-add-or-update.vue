...
<script lang="ts">
	...
	export default defineComponent({
		...
		methods: {
			...
			// 获取信息
			getInfo() {
				baseService.get("/programme/casesettinginstitute/" + this.dataForm.id).then((res) => {
					if (res.code !== 0) {
			...
			instituteChange(data ?: IObject) {
				if(data) {
					...
					// if(this.dataForm.id && this.ids) {
					// 	let tempDataForm = JSON.parse(JSON.stringify(this.dataForm));
					// 	for(let i = 0; i < this.ids.length; i++) {
					// 		console.log(this.dataForm.instituteNo);
					// 		if(this.ids[i].instituteNo != tempDataForm.instituteNo) {
					// 			if(data.number == this.ids[i].instituteNo) {
					// 				this.$message.error("当前学院已设置，请重新选择");
					// 				this.$refs["dataFormRef"].resetFields();
			...
			dataFormSubmitHandle() {
				this.$refs["dataFormRef"].validate((valid : boolean) => {
					...
					(!this.dataForm.id ? baseService.post : baseService.put)("/programme/casesettinginstitute", this.dataForm).then((res) => {
						if (res.code !== 0) {
							return this.$message.error(res.msg);
						}
						this.$message({
