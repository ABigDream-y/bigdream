<template>
	<div class="mod-sys__user">
		<el-form :inline="true" :model="dataForm" @keyup.enter="getDataList()">
			...
			<el-form-item>
				<el-button v-if="hasPermission('sys:user:save')" type="primary" @click="lecturerUpdateHandle()">
					{{ $t("add") }}
				</el-button>
			</el-form-item>
			<el-form-item>
		...
		<!-- 存在录入错误 -->
		<el-dialog v-model="isExistenceVisible" title="添加用户错误" :close-on-click-modal="false"
			:close-on-press-escape="false">
			<el-table :data="errorList" style="width: 100%;">
				<el-table-column prop="staffId" label="职工号" header-align="center" align="center"></el-table-column>
...
<script lang="ts">
	...
	import AddOrUpdate from "./user-lecturer.vue";
	export default defineComponent({
		...
		setup() {
			const state = reactive({
				errorList: [],
				isExistenceVisible: false,
				visible: false,
				createdIsNeed: false,
				activatedIsNeed: false,
				getDataListURL: "/sys/user/pageByRoleId",
		...
		activated() {
			console.log('dataForm', this.$route.query)
			this.init()
		},
		methods: {
			init() {
			...
			lecturerUpdateHandle() {
				this.isExistenceVisible = false;
				this.addOrUpdateVisible = true
				this.$nextTick(() => {
					// this.$refs.addOrUpdate.dataForm.id = id;
					this.$refs.addOrUpdate.init(this.dataForm.roleId)
				})
			},
			refreshDataList(data: any) {
				this.getDataList()
				this.addOrUpdateVisible = false;
				if (data && data.length != 0) {
					this.errorList = data;
					this.$nextTick(() => {
						this.isExistenceVisible = true;
					})
				}
			}
		}
