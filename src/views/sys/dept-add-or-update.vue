<template>
	<el-dialog v-model="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false"
		...
		<el-form :model="dataForm" :rules="dataRule" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()"
			...
			<el-form-item prop="number" label="学院代码">
				<el-input v-model.number="dataForm.number" placeholder="学院代码"></el-input>
				<p style="margin-top:5px;margin-bottom:0;">注意：除第一级外，学院代码必须以上级代码为前缀</p>
			</el-form-item>
			<el-form-item prop="pnumber" label="上级学院代码" v-if="dataForm.pid != '0'">
			...
			init() {
				...
				this.$nextTick(() => {
					...
					this.getDeptList().then(() => {
						if (this.dataForm.id) {
							this.getInfo();
						} else if (this.$store.state.user.superAdmin === 1 || this.$store.state
							.user.superTenant === 1) {
