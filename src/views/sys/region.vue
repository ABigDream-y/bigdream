<template>
	<div class="mod-sys__region">
		<el-form :inline="true" :model="dataForm" @keyup.enter="getDataList()">
			<el-form-item>
				<el-button v-if="hasPermission('sys:region:save')" type="primary" @click="addOrUpdateHandle()">
					{{ $t("add") }}</el-button>
			</el-form-item>
		</el-form>
		<el-table ref="elTable" v-loading="dataListLoading" :data="dataList" row-key="id" border lazy :load="load"
			...
			<el-table-column prop="sort" :label="$t('region.sort')" header-align="center" align="center"
				min-width="150"></el-table-column>
			<el-table-column prop="updateDate" :label="$t('region.updateDate')" header-align="center" align="center"
				min-width="150"></el-table-column>
			<el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
				<template v-slot="scope">
					<el-button v-if="hasPermission('sys:region:update')" type="text" size="small"
						@click="addOrUpdateHandle(scope.row.id)">{{ $t("update") }}</el-button>
					<el-button v-if="hasPermission('sys:region:delete')" type="text" size="small"
