<template>
	...
	<div>
		...
		<span><i class="el-icon-user-solid" style="color: #e6444a"></i>
			{{ $t("notice.senderName") }}：{{ dataForm.senderName }}</span>
		<el-divider direction="vertical"></el-divider>
		<span><i class="el-icon-time" style="color: #e6444a"></i>
	...
	<el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle"
		style="width: 100%">
		<el-table-column prop="receiverName" :label="$t('notice.receiverName')" header-align="center"
			align="center"></el-table-column>
		<el-table-column prop="readStatus" :label="$t('notice.readStatus')" header-align="center" align="center">
			<template v-slot="scope">
				<el-tag v-if="scope.row.readStatus === 0" size="small"
					type="danger">{{ $t("notice.readStatus0") }}</el-tag>
				<el-tag v-else size="small" type="success">{{ $t("notice.readStatus1") }}</el-tag>
			</template>
		</el-table-column>
		<el-table-column prop="readDate" :label="$t('notice.readDate')" header-align="center"
			align="center"></el-table-column>
	</el-table>
