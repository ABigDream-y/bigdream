...
const useView = (props: IViewHooksOptions | IObject): IViewHooks => {
	...
	const viewFns = {
		...
		query() {
			...
			baseService
				.get(state.getDataListURL, {
					pageNum: state.getDataListIsPage ? state.page : null,
					pageSize: state.getDataListIsPage ? state.limit : null,
					...state.dataForm
				})
				.then((res) => {
					...
					console.log('列表',res)
					if(res.data) {
						state.dataList = state.getDataListIsPage ? res.data.dataSet : res.data
						state.total = state.getDataListIsPage ? res.data.pageInfo.total : 0
					}else {
						state.dataList = [];
						state.total = 0;
					}
		...
		editHandle(id?: string) {
			console.log('useview-edithandle',state.getDataListURL)
			console.log('useview-edithandle',state.routeName)
