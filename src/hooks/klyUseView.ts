...
import { IObject, IFunction, IViewHooks, IViewHooksOptions } from "@/types/interface";
import { registerDynamicToRouterAndNext } from "@/router";
import baseService from "@/service/klyBaseService";
...
const useView = (props : IViewHooksOptions | IObject) : IViewHooks => {
	...
	const rejectFns = {
		// 表单是否是创建者
		isFormCreator(creator:string) {
			// console.log('user====',store.state.user)
			if(store.state.user.superAdmin == 1 || store.state.user.id == creator) {
				return true;
			}else {
				return false;
			}
	...
	const viewFns = {
		...
		query() {
			...
			baseService
				...
				.then((res) => {
					...
					if (state.returnDataKey == 'page') {
						state.dataList = state.getDataListIsPage ? res.page.list : res.data;
						state.total = state.getDataListIsPage ? res.page.totalCount : 0;
					} else if (state.returnDataKey == 'data') {
						state.dataList = state.getDataListIsPage ? res.data.list : res.data;
						state.total = state.getDataListIsPage ? res.data.totalCount : 0;
					}

					console.log('data', state.dataList)
					// this.dataList = this.mixinViewModuleOptions.getDataListIsPage ? res.data.dataSet : res.data
					// this.total = this.mixinViewModuleOptions.getDataListIsPage ? res.data.pageInfo.total : 0
				})
				.catch(() => {
					state.dataListLoading = false;
				});
		...
		editHandle(id ?: string) {
			...
			const routeParams = {
				...
				query: {
					id: id ? id : 0,
					_mt: `${self.$route.meta.title} - ${state.routeTitleCustom ? state.routeTitleCustom : (id ? '编辑' : '新增')}`
				}
			};
		...
		editWidthParamsHandle(query ?: IObject) {
			...
			const routeParams = {
				...
				query: {
					...query,
					_mt: `${self.$route.meta.title} - ${id ? '编辑' : '新增'}`
				}
			};
			registerDynamicToRouterAndNext(routeParams);
		},
		...
		deleteHandle(id ?: string, pid ?: string) : Promise<any> {
			return new Promise((resolve, reject) => {
				...
				ui.confirm(t("prompt.info", { handle: t("delete") }), t("prompt.title"), {
					...
					.then(() => {
						((state.deleteMethod && state.deleteMethod == 'post') ? baseService.post : ((state.deleteMethod && state.deleteMethod == 'get') ? baseService.get : baseService.delete))(
		...
		// 导出
		exportHandle() {
			window.location.href = `${app.klyApi}${state.exportURL}?${qs.stringify({
				...state.dataForm,
				token: getToken()
			})}`;
		...
		importHandle(){
			...
			window.location.href = `${app.klyApi}${state.importURL}?${qs.stringify({
				...state.dataForm,
				token: getToken()
			})}`;
		},
