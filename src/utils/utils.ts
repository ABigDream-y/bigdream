...
import { provinceData } from "@/utils/data/province";
import { cityData } from "@/utils/data/city";
import { areaData } from "@/utils/data/area";

// 获取省市区数据
export function getAreaData(same:boolean) {
	let option : IObject = provinceData;

	option.forEach((item : IObject, index : number) => {
		item['children'] = cityData[index] || [];
		if(same) {
			item.value = item.label;
		}
		if (item.children) {
			item.children.forEach((cItem : IObject, cIndex : number) => {
				if(same) {
					cItem.value = cItem.label;
				}
				cItem['children'] = areaData[index][cIndex] || [];
				if(same) {
					cItem.children.forEach((ccItem : IObject) => {
						ccItem.value = ccItem.label;
					})
				}
			})
		}
